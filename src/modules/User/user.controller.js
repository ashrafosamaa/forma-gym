...
export const getAllUsers = async (req, res, next) => {
    ...
    const features = new APIFeatures(req.query, User.find().select("firstName lastName phoneNumber email memberStatus gender"))
...
export const getUser = async(req, res, next)=> {
    ...
    // get user data
    const getUser = await User.findById(userId).select("firstName lastName phoneNumber email memberStatus gender profileImg weight height")
    if (!getUser) {
...
export const updateUser = async(req, res, next)=> {
    // destruct data from user
    const {userId} = req.params
    const {firstName, lastName, phoneNumber, weight, height, memberStatus} = req.body
    // find user
    const user = await User.findById(userId).select("firstName lastName phoneNumber email memberStatus gender profileImg.secure_url weight height")
    // check user
    if(!user){
        return next (new Error("User not found", { cause: 404 }))
    }
    ...
    if(firstName || lastName || weight || height || memberStatus){
        ...
        user.memberStatus = memberStatus ?? user.memberStatus
    }
    await user.save()
    // send response
    res.status(200).json({
...
export const updateProfileData = async (req, res, next)=> {
    ...
    // update user data
    if(firstName || lastName || phoneNumber || weight || height){
        user.firstName = firstName ?? user.firstName
        user.lastName = lastName ?? user.lastName
        user.phoneNumber = phoneNumber ?? user.phoneNumber
        user.weight = weight ?? user.weight
